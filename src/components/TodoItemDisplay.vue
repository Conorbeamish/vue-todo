<template>
    <div :class="['bg-white p-4 rounded shadow mb-2 flex items-center', { 'flex-col': isEditing }]">
      <button @click="toggleEdit" name="edit" class=" mr-2">
      ...
      <div class="flex space-x-2 mt-2 justify-end">
        <button @click="updateComplete" name="complete" class="text-green-500 hover:text-green-700">
          {{ todo.completed ? 'Undo' : 'Complete' }}
        </button>
        <button @click="deleteTodoItem" name="delete" class="text-red-500 hover:text-red-700">Delete</button>
      </div>
    </div>
  </template>
